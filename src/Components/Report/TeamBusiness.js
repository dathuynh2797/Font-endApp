...
export class TeamBusiness extends React.Component {
  constructor(props) {
    super(props);
  ...
  handleTeam = item => {
    ...
    this.state.AllDoanhSo.forEach(e => {
      if (item.Id === e.Id) {
        ...
        arrY.splice(arrY.length);
        arrD.splice(arrD.length);
      }
    });
    arrY.forEach((e, i) => {
  ...
  componentDidMount = async () => {
    ...
    try {
      allUserSnapShot.forEach(doc => {
        if (doc.data().unitsTitle !== 'Ban Giám Đốc') {
          ...
          //   arrDoanhSo.push({Id: doc.id, DoanhSo: arrData.slice()});
        }
      });
    ...
    for (let i = 0; i < arrData.length; i++) {
      let value = Object.entries(arrData[i]);
      let arrNew = value.slice(0, value.length - 79);
      arrDoanhSo.push({Id: arrData[i].id, DoanhSo: Object.fromEntries(arrNew)});
    }
  ...
  render() {
    const {User, Team, Year, Quarter, Mounth, userChart} = this.state;
